...
import { defineNuxtModule, addPlugin, createResolver, addImports, addServerHandler, addTemplate, addServerPlugin } from '@nuxt/kit'
...
function createRequiredError(field: string) {
  throw new Error(`[nuxt-auth] config option '${field}' is required`)
}

function setupFrontend(options: ModuleOptions, nuxt: Nuxt) {
...
function setupBackend(options: ModuleOptions, nuxt: Nuxt) {
  if (!options.backendEnabled) {
